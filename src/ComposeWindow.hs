...
mailSender smtpserver username password rcptAddrList ccAddrList bccAddrList subject msg attachList = 
	let 
	...
	in 
		do
			attachments <- unravel attachList
...
main = do
	...
	readConfig "smtpserver"$ \smtpserver ->
	...
	onClicked sendmailButton $ do
		...
		if length (rcptAddrList++ccAddrList++bccAddrList) > 0
			then do
				-- Send mail
				result <- mailSender smtpserver username password rcptAddrList ccAddrList bccAddrList subjectAddr bodyAddr filenames
